...
class Siswa extends CI_Controller 
{
	...
	function index($offset=0,$order_column='id',$order_type='asc')
	{
		...
		$this->table->set_heading('No',anchor('siswa/index/'.$offset.'/nama/'.$new_order,'Nama'),anchor('siswa/index/'.$offset.'/alamat/'.$new_order,'Alamat'),anchor('siswa/index/'.$offset.'/jenis_kelamin/'.$new_order,'Jenis Kelamin'),anchor('siswa/index/'.$offset.'/tanggal_lahir/'.$new_order,'Tanggal Lahir (dd-mm-yyyy)'),'Actions');

		$i=0+$offset;
		foreach ($siswas as $siswa)
		{
			$this->table->add_row(++$i,$siswa->nama,$siswa->alamat,strtoupper($siswa->jenis_kelamin)=='M'?'Laki-Laki':'Perempuan',date('d-m-Y',strtotime($siswa->tanggal_lahir)),anchor('siswa/view/'.$siswa->id,'view',array('class'=>'view')).' '.anchor('siswa/update/'.$siswa->id,'update',array('class'=>'update')).' '.anchor('siswa/delete/'.$siswa->id,'delete',array('class'=>'delete','onclick'=>"return confirm('Apakah Anda yakin ingin menghapus data siswa?')")));
		}
